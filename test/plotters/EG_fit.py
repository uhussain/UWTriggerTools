...
eff_file = ROOT.TFile("../data/Evan/uct_eg_efficiency_eic3.root")

eg_eff_ntuple = eff_file.Get("rlxEGEfficiency/Ntuple")
eg_eff_ntuple_cur = eff_file.Get("rlxEGEfficiency/Ntuple")
saveWhere='../plots/EG_'
...
for pt,color in zip(l1ptVal,colors):
 ...
 cutN=IDCut+'&&'+TauCut+'&&'+l1gPtCut
 cutC=IDCut+'&&'+l1PtCut
  
 resultIso = make_turnon(eg_eff_ntuple,cutI,color,'pt%i_Iso'%pt,save=True)
 resultNon = make_turnon(eg_eff_ntuple,cutN,color,'pt%i_Non'%pt,save=True)
 resultCur = make_turnon(eg_eff_ntuple_cur,cutC,color,'pt%i_Cur'%pt,save=True)
