...
TauVetoRlx = True #apply tau veto to relaxed taus (should be set to false by default)
...
turnOffIso = 63
selection = "TauVetoAndNeighbor"
turnOffTauVeto = 60
L1_CALIB_FACTOR = 1.0
L1G_CALIB_FACTOR = 1.0
#ZEROBIAS_RATE=15000000.00
ZEROBIAS_RATE = 11246.0*2590.0 #frequency X bunches
saveWhere = 'plots/new_4x8vs4x4E3H3/Rate_new_Tau4x8'+selection+'RlxVeto'+str(TauVetoRlx)+'Apply4x4TauVeto'+str(Veto4x4)+'OffIso'+str(turnOffIso)+'OffVeto'+str(turnOffTauVeto)+'Iso'+str(ISOTHRESHOLD)
if Cmp4x4 == False:
	saveWhere = 'plots/new_ECAL3HCAL3/Rate_new_Tau4x8'+selection+'RlxVeto'+str(TauVetoRlx)+'OffIso'+str(turnOffIso)+'OffVeto'+str(turnOffTauVeto)+'Iso'+str(ISOTHRESHOLD)
...
########
#Rate
rate_ntuple = 'tau_rate_4x8_new_calibrations_July30.root'
...
# UCT Relaxed
 if drawUCTRlx_:
  #if TauVetoRlx == False:\\UCT Rlx with no tau veto
 ...
 # else://UCT Rlx with tau veto
   cut_UCT_rlx_1=extraCut
   rlxLegendStr_1 = 'UCT (4x8): Rlx (Tau Veto)'
   h_UCT_rlx_1,max_UCT_rlx_1,vert_UCT_rlx_1,hor_UCT_rlx_1,uctrlxRateVal_1 = rate_histo(
   ntuple_UCT_rlx,cut_UCT_rlx_1,binning,L1G_CALIB_FACTOR,
   scale,colorCI,markerCI,legend,
   rlxLegendStr_1,
   log,line,ptLine,aUR,bUR)
